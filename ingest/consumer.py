...
def write_batch(con: duckdb.DuckDBPyConnection, batch: list[dict]) -> None:
    ...
    Dùng ON CONFLICT DO UPDATE thay vì INSERT thuần để đảm bảo idempotent:
    - event_id là PRIMARY KEY nên không trùng
    - Nếu message được replay với nội dung đã đổi, DO UPDATE sẽ cập nhật thay vì bỏ qua
    - Nếu message được replay với cùng nội dung, DO UPDATE không thay đổi gì (idempotent)
    """
    con.executemany(
        f"insert into {TABLE} values (?, ?, ?, ?, ?, ?, ?, ?)"
        f" on conflict (event_id) do update set"
        f" ticket_id=excluded.ticket_id,"
        f" customer_id=excluded.customer_id,"
        f" customer_name=excluded.customer_name,"
        f" event_type=excluded.event_type,"
        f" latency_ms=excluded.latency_ms,"
        f" event_time=excluded.event_time,"
        f" _ingested_at=excluded._ingested_at",
        [
            (
...
def consume(
    ...
    with LogConsumer(topic, offset_file) as consumer:
        ...
        while True:
            ...
            # Đổi thứ tự: ghi trước, commit sau → at-least-once semantics
